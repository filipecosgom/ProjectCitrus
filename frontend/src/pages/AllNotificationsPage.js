...
export default function AllNotificationsPage() {
  ...
  const fetchAndSetNotifications = useNotificationStore((state) => state.fetchAndSetNotifications);
  ...
  useEffect(() => {
    ...
    all.sort((a, b) => {
      const aTime = Array.isArray(a.timestamp) ? new Date(...a.timestamp.slice(0, 6)) : new Date(a.timestamp);
      const bTime = Array.isArray(b.timestamp) ? new Date(...b.timestamp.slice(0, 6)) : new Date(b.timestamp);
      return bTime - aTime;
    });
