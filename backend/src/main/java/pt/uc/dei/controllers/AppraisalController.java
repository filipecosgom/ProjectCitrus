...
public class AppraisalController {
    ...
    public Response updateAppraisal(UpdateAppraisalDTO updateAppraisalDTO, @CookieParam("jwt") String jwtToken) {
        try {
            ...
            if (validationResponse != null && !isAdmin) {
                ...
                if (updateAppraisalDTO.getState() == AppraisalState.COMPLETED) {
                    if ((updateAppraisalDTO.getFeedback().trim() == null || updateAppraisalDTO.getFeedback().trim().isEmpty())
                            || updateAppraisalDTO.getScore() == null || updateAppraisalDTO.getScore() < 1 || updateAppraisalDTO.getScore() > 4) {
                        if (!isAdmin) {
                            return Response.status(Response.Status.BAD_REQUEST)
                                    .entity(new ApiResponse(false, "Feedback or score must not be blank for completed appraisal",
                                            "errorFeedbackBlank", null))
                                    .build();
                        }
                    }
                }
                try {
                    ...
                    AppraisalDTO updatedAppraisal = appraisalService.updateAppraisal(updateAppraisalDTO, isAdmin);
